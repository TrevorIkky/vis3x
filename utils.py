...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        crop_idxs = torch.cumsum(torch.unique_consecutive(L, return_count=True)[1], 0)
...
def cosine_scheduler(base_val, final_val, epochs, n_iters_per_epoch, warmup_epochs=0, warmup_val=0):
    ...
    iters = (epochs - warmup_epochs) * n_iters_per_epoch
    schedule = final_val + 0.5 * (base_val - final_val) * (1 + np.cos(iters / (len(iters))) * np.pi)
